...
direction nearly eliminates refusal of harmful
instructions.  This is a striking
...
0.10$).  This is a concrete instance of SVD ordering
not coinciding with the causally relevant subspace:
at low $\lambda$, the top principal directions of
$M_{\mathcal{D}_s}$ are not the directions whose
removal degrades compliance; at $\lambda{=}50$ the
regularizer has pushed the alignment shift to span a
larger subspace whose top directions \emph{do} contain
the compliance-relevant components, and the same
projection ablates them.  The present experiment does
not localize a separate ``safety circuit'' to confirm
either mechanism; the MLP serves as a controlled
instance where variance ordering and causal ordering
can be made to coincide or diverge by tuning a single
hyperparameter.
...
A chat-template confound (the aligned model is
evaluated under chat formatting while the base model
has not seen chat tokens) requires comparing four
matrices under matched collection conditions:
...
\paragraph{Decomposing the $k=5$ rebound.}
...
table decomposes the rebound: $u_1$ alone drops
refusal to $0.84$; $u_2$ alone is null; $u_3$--$u_5$
are individually weak ($\sim 0.94$).  At $k=3$ the
ablation already covers $u_1$ together with the inert
$u_2$ and one of the
...
cover.  This reading aligns with the concept-cone picture of
\citet{wollschlager2025} and with the wide-band
$k=20$ result that collapses refusal to $\mathbf{0.00}$,
but the present per-direction data alone does not
distinguish it from alternatives (keyword-classifier
interaction with specific token distributions at
$k=5$, layer-interaction effects across the narrow
band); combination ablations on non-consecutive
subsets of the top-$k$ block, which we do not run
here, would be required to verify.  Either way, Test~2
must be evaluated as a curve in $k$ and the SVD
...
One might argue the two-stage paradigm
...
concept \emph{cones}---multiple mechanistically
independent directions, not a single one---and our
centered-SVD residual rank $\sim 75$--$80$ on
$M_{\mathcal{D}_s}$ matches this multi-dimensional
picture rather than being independent of it: we
re-measure the multi-dimensional structure under the
four-variant control, but the existence of
multi-dimensional structure beyond the rank-$1$
direction is established by \citet{wollschlager2025}.
...
\citet{leike2018scalable, amodei2016concrete,
irving2018ai} seek scalable oversight; \citet{wolf2024fundamental}
proves behavioral lower bounds on adversarial elicitation under
any partial alignment;
...
All experiment code, prompt sets, and pre-computed JSON results
used in this paper are publicly available at
\url{https://github.com/Nakammura/effective-rank-audit} under the
CC-BY-4.0 license.  The repository contains the LaTeX source of
this paper; the diagnostic pipeline
(\texttt{diagnostics\_v4.py}); the calibration MLP
(\texttt{calibration\_v2.py},
\texttt{calibration\_v2\_revision3.py},
\texttt{calibration\_kappa.py}); the supplementary experiments
(\texttt{revision3.py}, \texttt{revision6\_experiments.py});
Modal wrappers for cloud-GPU reproduction
(\texttt{modal\_revision3.py}); and the LLM-judge cross-check
implementation (\texttt{llm\_judge\_eval.py}).  An immutable
snapshot of the v1.0 release is archived on Zenodo:
\href{https://doi.org/10.5281/zenodo.20341445}{\texttt{10.5281/zenodo.20341445}}
(concept DOI for all versions:
\href{https://doi.org/10.5281/zenodo.20341444}{\texttt{10.5281/zenodo.20341444}}).
